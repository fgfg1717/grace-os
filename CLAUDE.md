...
- **運動**：App → 🏋️ 運動追蹤 → 選健身房/居家打卡
- **週覆盤/月回顧**：不在 App 內，改用 Claude `/週復盤`、`/月復盤` slash command（2026/08/21 App 內的今日策略/週覆盤/月回顧三頁已刪除，她說用不到）
...
| 📚 英文學習中心 | 查記/手動新增、SRS 間隔複習、每日新詞、練習 | `eng_items_v1` |
| 🎯 幕僚沙盤 | iframe 嵌入，連到獨立 App | — |
| 🏋️ 運動追蹤 | 動作等級打卡（健身房/居家）、週/月/長期趨勢檢視 | `gos_workout_checkins`, `gos_workout_goals` |

## 英文學習中心架構（SRS，2026/08/14 改版）
- **沒有講義/課程匯入了**：舊的 JSON 匯入（自動背景匯入 + `data/lessons/` 資料夾 + 課程篩選 chips）已整個移除，不是關閉、是真的刪掉了
- **統一學習池**：所有來源（查字典、老師補充、手動、每日新詞）全進 `eng_items_v1`，每筆自帶 `item.srs` 子物件記錄複習狀態，不需要另一個 SRS 專屬 store
- **SRS 間隔複習**：4 級自評（不會=1天、模糊=3天、會了=7天起 ×1.6、很熟=14天起 ×2），`getDueEngItems()` 抓到期項目、`updateItemSRS(itemId,rating)` 更新
- **內建詞庫**：150+ 高頻詞，存在 `TOEIC_WORDS` 常數（變數名保留未動，畫面顯示是「每日新詞」），每日推 5 個新詞，複習卡刷完才出現，可選擇加入 `eng_items_v1`
- **今日 tab 邏輯**：有待複習（含所有還沒被複習過的舊紀錄）→ 5 張一批刷複習卡；無待複習 → 顯示今日每日新詞
...
- **「今日策略」**：自動執行以下步驟：
  1. 確認今天日期（Asia/Taipei）和星期幾
  2. WebFetch GET GAS API 抓本週計畫（type=plan）
  3. 找出今天的排程項目列出來
  4. 依序問三個問題（等使用者回答）：
     - 今天最重要的一件事是什麼？
     - 有沒有潛在障礙要提前想好應對？
     - 確認今日必完成 1-3 件事（可從計畫中挑）
  5. 整理成今日執行摘要輸出，結尾一句簡短鼓勵
- **「週覆盤」**：自動執行以下步驟：
